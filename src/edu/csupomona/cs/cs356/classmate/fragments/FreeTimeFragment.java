	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		client.get("http://www.lol-fc.com/classmate/getfriends.php", params, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(String response) {
				...
				if (1 < response.length()) {
					try {
						...
						for (int i = 0; i < myjsonarray.length(); i++) {
							...
							friends.add(new Friend(jObj.getLong("friend_id"), jObj.getString("fusername"), jObj.getString("femail")));
						}
				...
				FreeTimeAdapter adapter = new FreeTimeAdapter(getActivity(), friends);
